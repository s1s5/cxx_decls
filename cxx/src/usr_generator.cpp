 ...
 * @file usr_generator.cpp
 * @brief
 * @author shogo.sawai@gmail.com (Shogo Sawai)
 * @date 2016-05-26 09:55:36
 */
...
class USRGenerator : public ConstDeclVisitor<USRGenerator> {
 ...
 public:
    explicit USRGenerator(ASTContext *Ctx, SmallVectorImpl<char> &Buf)
        : Buf(Buf), Out(Buf), IgnoreResults(false), Context(Ctx), generatedLoc(false) {
    ...
    void VisitLinkageSpecDecl(const LinkageSpecDecl *D) { IgnoreResults = true; }

    void VisitUsingDirectiveDecl(const UsingDirectiveDecl *D) { IgnoreResults = true; }

    void VisitUsingDecl(const UsingDecl *D) { IgnoreResults = true; }

    void VisitUnresolvedUsingValueDecl(const UnresolvedUsingValueDecl *D) { IgnoreResults = true; }

    void VisitUnresolvedUsingTypenameDecl(const UnresolvedUsingTypenameDecl *D) { IgnoreResults = true; }

    bool ShouldGenerateLocation(const NamedDecl *D);

    bool isLocal(const NamedDecl *D) { return D->getParentFunctionOrMethod() != nullptr; }
    ...
    // void GenObjCClass(StringRef cls) { generateUSRForObjCClass(cls, Out); }

    /// Generate a USR for an Objective-C class category.
    // void GenObjCCategory(StringRef cls, StringRef cat) { generateUSRForObjCCategory(cls, cat, Out); }

    /// Generate a USR fragment for an Objective-C property.
    // void GenObjCProperty(StringRef prop) { generateUSRForObjCProperty(prop, Out); }

    /// Generate a USR for an Objective-C protocol.
    // void GenObjCProtocol(StringRef prot) { generateUSRForObjCProtocol(prot, Out); }
...
void USRGenerator::VisitFunctionDecl(const FunctionDecl *D) {
    ...
    if ((!Ctx.getLangOpts().CPlusPlus || D->isExternC()) && !D->hasAttr<OverloadableAttr>())
        return;
    ...
    for (auto && PD = D->param_begin(), e = D->param_end(); PD != e; PD++) {
        Out << '#';
...
void USRGenerator::VisitObjCMethodDecl(const ObjCMethodDecl *D) {
    ...
    Out << (D->isInstanceMethod() ? "(im)" : "(cm)") << DeclarationName(D->getSelector());
}
...
bool USRGenerator::GenLoc(const Decl *D, bool IncludeOffset) {
    ...
    IgnoreResults = IgnoreResults || printLoc(Out, D->getLocStart(), Context->getSourceManager(), IncludeOffset);
...
#include "clang/AST/BuiltinTypes.def"
        ...
        // encoding.
        llvm::DenseMap<const Type *, unsigned>::iterator Substitution = TypeSubstitutions.find(T.getTypePtr());
...
void USRGenerator::VisitTemplateParameterList(const TemplateParameterList *Params) {
    if (!Params)
        return;
    Out << '>' << Params->size();
    for (TemplateParameterList::const_iterator P = Params->begin(), PEnd = Params->end(); P != PEnd; ++P) {
        Out << '#';
